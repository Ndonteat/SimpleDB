...
//    int pageSize;
    ...
    */
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                if (tidToFirstLogRecord.containsKey(tid.getId())){
                    logRecordId = tidToFirstLogRecord.get(tid.getId());
                } else {
                    return;
                }
                ...
                long end = currentOffset;

                if (end == NO_CHECKPOINT_ID) {
                    end = raf.length();
                }


                Map<PageId, Boolean> finishedMap = new HashMap<>();
                //Set<PageId> finished = new HashSet<>();
                raf.seek(logRecordId);
                ...
                while (raf.getFilePointer() < end) {
                    ...
                    if (type == CHECKPOINT_RECORD){
                        int i = 0;
                        while (i < raf.readInt()){
                            raf.seek(raf.getFilePointer() + LONG_SIZE * 2);
                            i++;
                        }
                    } else if (type == UPDATE_RECORD){
                        Page before = readPageData(raf);
                        if (currentId == tid.getId()) {

                            PageId pid = before.getId();
                            if (!finishedMap.containsKey(pid)) {
                                finishedMap.put(pid, true);
                                this.writeAndDiscard(before, pid);
                            }
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                if (tidToFirstLogRecord.containsKey(tid)){
                    logRecordId = tidToFirstLogRecord.get(tid);
                } else {
                    return;
                }
                ...
                long start = raf.getFilePointer();
                long end = currentOffset;
                if (currentOffset == NO_CHECKPOINT_ID) {
                    end = raf.length();
                }
    ...
    private void writeAndDiscard(Page before, PageId pid) throws IOException {
    ...
    */
    ...
    private long checkCheckPoint(long lastcheckpoint){
    ...
    private long relocatCurrentTid(long currentTid) throws IOException {
        if (currentTid <= LONG_SIZE) {
            currentTid = -1;
        } else {
            raf.seek(currentTid - LONG_SIZE);
            currentTid = raf.readLong();
        }
        return currentTid;
    }

    private long setCurrentTid() throws IOException {
        raf.seek(currentOffset - LONG_SIZE);
        return raf.readLong();
    }

    private long setLastCheckPoint() throws IOException {
        raf.seek(0);
        return raf.readLong();
    }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Map<Long, Boolean> transactionMap = new HashMap<>();
                Map<Long, Boolean> finishedMap = new HashMap<>();

                currentOffset = raf.length();
                long currentTid = this.setCurrentTid();
                long lastCheckPoint = this.setLastCheckPoint();


                // What happens if no checkpoint
                lastCheckPoint = this.checkCheckPoint(lastCheckPoint);

                    // backward start from last record
                while (currentTid >= lastCheckPoint) {
                    raf.seek(currentTid);
                    int type = raf.readInt();
                    long recordTid;
                    ...
                    if (type == COMMIT_RECORD){
                        ...
                        finishedMap.put(recordTid, true);
                    }
                    if (type == BEGIN_RECORD){
                        recordTid = raf.readLong();
                        transactionMap.put(recordTid, true);
                    }
                    if (type == CHECKPOINT_RECORD){
                        if (lastCheckPoint == currentTid){
                            raf.seek(raf.getFilePointer() + LONG_SIZE);

                            int activeNumber = raf.readInt();
                            int i = 0;
                            while (i < activeNumber){
                                i++;
                                recordTid = raf.readLong();
                                long currendLogRecord = raf.readLong();
                                transactionMap.put(recordTid, true);
                                tidToFirstLogRecord.put(recordTid, currendLogRecord);
                            }
                        } else {
                            throw new IOException("CKP error!");
                        }

                    }
                        // relocate current tid
                        currentTid = this.relocatCurrentTid(currentTid);
                    }


                    currentTid = lastCheckPoint;
                    while (currentTid < currentOffset) {
                        raf.seek(currentTid);
                        int type = raf.readInt();
                        long recordTid;
                        ...
                        if (type == UPDATE_RECORD){
                            ...
                            this.writeAndDiscard(after, after.getId());
                            currentTid = raf.getFilePointer();
                        }
                        if (type == ABORT_RECORD){
                            recordTid = raf.readLong();
                            if (tidToFirstLogRecord.get(recordTid) == null){
                                throw new IOException("abort record some thing wrong");
                            } else {
                                currentTid = raf.getFilePointer();
                                rollback(recordTid);
                                transactionMap.remove(recordTid);
                            }
                        ...
                        if (type == BEGIN_RECORD){
                            recordTid = raf.readLong();
                            tidToFirstLogRecord.put(recordTid, currentTid);
                            currentTid = raf.getFilePointer();
                        }
                        if (type == CHECKPOINT_RECORD){
                            if (lastCheckPoint == currentTid){
                                ...
                                currentTid = raf.getFilePointer() + numActive * LONG_SIZE * 2;
                            } else {
                                throw new IOException();
                            }
                        ...
                        currentTid += LONG_SIZE;
                    }

                    // undo
                    for (Long tid : transactionMap.keySet()) {
                        if (!finishedMap.containsKey(tid)) {
                            if (tidToFirstLogRecord.containsKey(tid)){
                                rollback(tid);
                            } else {
                                throw new IOException();
