...
public class Worker {
    ...
    public boolean isRunning() {
        System.out.println("queryPlan: " + this.queryPlan);
    ...
    public void localizeQueryPlan(OpIterator queryPlan) {
        ...
        if (queryPlan instanceof SeqScan){
            localizeSeqScan(queryPlan);
        } else if (queryPlan instanceof Operator) {
            choosePlan(queryPlan);
            OpIterator[] children = ((Operator) queryPlan).getChildren();
            traverseLocalize(children);
        }
    }

    private void localizeSeqScan(OpIterator queryPlan){
        ...
        String alias = seqScan.getAlias();
        seqScan.reset(tableid, alias);
    }

    private void choosePlan(OpIterator queryPlan){
    ...
    private void traverseLocalize(OpIterator[] children){
        int i = 0;
        while (i < children.length) {
            localizeQueryPlan(children[i]);
            i++;
        }
    }
